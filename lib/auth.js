...
// Підтримує декілька дійсних паролів (APP_PASSWORDS, через кому/з нового рядка)
// або один старий (APP_PASSWORD) — має збігатися з тим, що в app/api/login/route.js.
const APP_SECRET = process.env.APP_PASSWORDS || process.env.APP_PASSWORD || "";

function verifyLocalToken(token) {
  if (!APP_SECRET || !token || !token.includes(".")) return null;
